import { ApiConnectionError, escapeHtml, parseGameState, persistSession, readNumber, readSession, requestJson } from "./client.js";
import { GAMES, STAT_LABELS } from "./catalog.js";
import type { GameGuide, GameId, GameState, Phase, PlayerState, Session } from "./types.js";
...
function connectRoomSocket(): void {
  ...
  roomSocket.onmessage = (event) => {
    try {
      state = parseGameState(event.data);
      selectedGame = state.gameId;
      serverStatus = "online";
      render();
    } catch (error) {
      message = error instanceof Error ? error.message : "The server sent an invalid update.";
      roomSocket?.close();
    }
  };
...
async function api<T>(path: string, options: RequestInit = {}): Promise<T> {
  try {
    const body = await requestJson<T>(path, options);
    serverStatus = "online";
    return body;
  } catch (error) {
    if (error instanceof ApiConnectionError) serverStatus = "offline";
    throw error;
  }
}
...
function renderPhaseRail(phase: Phase): string {
  ...
  return `<nav class="phase-rail" aria-label="Game progress">${order.map((item, index) => `<span class="${index === current ? "current" : index < current ? "done" : ""}"><i>${index < current ? "✓" : index + 1}</i>${item === "results" ? "Reveal" : item === "finished" ? "Final" : item.charAt(0).toUpperCase() + item.slice(1)}</span>`).join("")}</nav>`;
}
